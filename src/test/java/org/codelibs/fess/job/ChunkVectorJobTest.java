...
public class ChunkVectorJobTest extends UnitFessTestCase {
    ...
    private ChunkVectorJob chunkVectorJob;
    ...
    protected void setUp(TestInfo testInfo) throws Exception {
        ...
        tempDir = File.createTempFile("chunkvectorjob_test", "");
        tempDir.delete();
        tempDir.mkdirs();
        ...
        // Register mock components before creating ChunkVectorJob
        ...
        // ComponentUtil fallback registry, so resolve the test mock via the seam.
        chunkVectorJob = new ChunkVectorJob() {
            @Override
    ...
    private void createRequiredDirectories() {
        // Create necessary directories for ChunkVectorJob execution
        new File(tempDir, "WEB-INF/lib").mkdirs();
        new File(tempDir, "WEB-INF/env/chunk/lib").mkdirs();
    ...
    public void test_execute_success() {
        ...
        mockProcessHelper.setOutput("Chunk vector indexing successful");

        String result = chunkVectorJob.execute();

        assertNotNull(result);
        assertTrue(result.contains("Session Id:"));
        assertNotNull(chunkVectorJob.sessionId);
        assertEquals(15, chunkVectorJob.sessionId.length());
    }
    ...
    public void test_execute_failure() {
        ...
        mockProcessHelper.setOutput("Chunk vector indexing failed");
    ...
    public void test_execute_withExistingSessionId() {
        ...
        chunkVectorJob.sessionId(existingSessionId);
        ...
        assertEquals(existingSessionId, chunkVectorJob.sessionId);
    }
    ...
    public void test_execute_withJobExecutor() {
        ...
        chunkVectorJob.jobExecutor(mockJobExecutor);
    ...
    @Test
    public void test_executeChunkVectorIndexer_withConfPath() {
        createRequiredDirectories();
    ...
    public void test_executeChunkVectorIndexer_withLocalFesen() {
        createRequiredDirectories();
        chunkVectorJob.useLocalFesen(true);
    ...
    @Test
    public void test_executeChunkVectorIndexer_commandAssembly() {
        createRequiredDirectories();
        chunkVectorJob.sessionId("CHUNKSESSION123");
        ...
        assertTrue("Command list should contain the ChunkVectorIndexer main class",
                cmdList.contains(ChunkVectorIndexer.class.getCanonicalName()));
        assertTrue("Command list should mark the chunk process", cmdList.contains("-Dfess.chunk.process=true"));
        int sessionIndex = cmdList.indexOf("--sessionId");
        assertTrue("Command list should contain --sessionId", sessionIndex >= 0);
        assertEquals("CHUNKSESSION123", cmdList.get(sessionIndex + 1));
        int cpIndex = cmdList.indexOf("-cp");
        assertTrue("Should have -cp argument", cpIndex >= 0);
        String classpath = cmdList.get(cpIndex + 1);
        assertTrue("Classpath should contain WEB-INF/env/chunk/resources",
                classpath.contains("WEB-INF" + File.separator + "env" + File.separator + "chunk" + File.separator + "resources"));
    }

    // Test executeChunkVectorIndexer applies jvm.chunk.options
    @Test
    public void test_executeChunkVectorIndexer_withJvmChunkOptions() {
        createRequiredDirectories();
        mockFessConfig.setJvmChunkOptions("-Xms128m -Xmx256m");
        ...
        List<String> cmdList = mockProcessHelper.getLastCommandList();
        assertNotNull(cmdList);
        assertTrue("Command list should contain -Xms128m from jvm.chunk.options", cmdList.contains("-Xms128m"));
        assertTrue("Command list should contain -Xmx256m from jvm.chunk.options", cmdList.contains("-Xmx256m"));
    }

    // Test executeChunkVectorIndexer with interrupted exception
    @Test
    public void test_executeChunkVectorIndexer_withInterruptedException() {
        createRequiredDirectories();
    ...
    public void test_sessionIdGeneration() {
        assertNull(chunkVectorJob.sessionId);

        mockProcessHelper.setExitValue(0);
        chunkVectorJob.execute();

        assertNotNull(chunkVectorJob.sessionId);
        assertEquals(15, chunkVectorJob.sessionId.length());
        // Check that session ID contains only alphabetic characters
        assertTrue(chunkVectorJob.sessionId.matches("[a-zA-Z]+"));
    }

    // Test command list construction with target classes directory
    @Test
    public void test_executeChunkVectorIndexer_withTargetClassesDir() {
        createRequiredDirectories();
    ...
    private class MockProcess extends Process {
        ...
        public java.io.InputStream getInputStream() {
            // JobProcess wraps this stream in an InputStreamThread; null would NPE
    ...
    private class MockFessConfig extends TestFessConfig {
        ...
        private String jvmChunkOptions = "";
        private boolean useOwnTmpDir = false;

        public void setJvmChunkOptions(String options) {
            this.jvmChunkOptions = options;
        }
        ...
        @Override
        public String[] getJvmChunkOptionsAsArray() {
            return jvmChunkOptions.isEmpty() ? new String[0] : jvmChunkOptions.split(" ");
        }
