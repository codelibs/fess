...
/**
 * Unit tests for {@link RelatedQueriesHandler}.
 *
 * <p>Verifies the wire contract for {@code GET /api/v2/related-queries}:
 * queries are returned when the helper has matches, an empty array is returned
 * for blank query, and non-GET is rejected with 405.</p>
 */
public class RelatedQueriesHandlerTest extends UnitFessTestCase {

    @BeforeEach
    public void registerStubHelpers() {
        // VirtualHostHelper is required by RelatedQueryHelper.
    ...
    /** Task 1.3: non-GET is rejected with 405. */
    @Test
    public void test_rejectsNonGet() throws Exception {
        final CapturingResponse res = new CapturingResponse();
        new RelatedQueriesHandler().handle(new StubRequest("POST", "/api/v2/related-queries", Collections.emptyMap()), res);
    ...
    /** Task 1.3: blank query returns empty queries array (no error). */
    @Test
    public void test_blankQueryReturnsEmptyArray() throws Exception {
        final CapturingResponse res = new CapturingResponse();
        new RelatedQueriesHandler().handle(new StubRequest("GET", "/api/v2/related-queries", Collections.emptyMap()), res);
        assertEquals(200, res.status);
        final String body = res.body();
        assertTrue(body.contains("\"status\":0"), body);
        assertTrue(body.contains("\"queries\":[]"), body);
    }

    /**
     * Task 1.3: when RelatedQueryHelper returns queries they are present in the response.
     * We register a stub helper that returns a fixed array so the test does not need
     * a live search backend.
     */
    @Test
    public void test_queriesReturnedFromHelper() throws Exception {
        final RelatedQueryHelper stubHelper = new RelatedQueryHelper() {
            @Override
            public String[] getRelatedQueries(final String query) {
                if ("fess".equals(query)) {
                    return new String[] { "fess search", "fess config" };
                }
                return new String[0];
            }
        };
        ComponentUtil.register(stubHelper, "relatedQueryHelper");
        try {
            final CapturingResponse res = new CapturingResponse();
            final Map<String, String[]> params = new HashMap<>();
            params.put("q", new String[] { "fess" });
            new RelatedQueriesHandler().handle(new StubRequest("GET", "/api/v2/related-queries", params), res);
            assertEquals(200, res.status, res.body());
            final String body = res.body();
            assertTrue(body.contains("\"status\":0"), body);
            assertTrue(body.contains("\"queries\""), body);
            assertTrue(body.contains("fess search"), body);
            assertTrue(body.contains("fess config"), body);
        } finally {
            // Reset to avoid polluting sibling tests.
            ComponentUtil.register(new RelatedQueryHelper(), "relatedQueryHelper");
        }
    }
