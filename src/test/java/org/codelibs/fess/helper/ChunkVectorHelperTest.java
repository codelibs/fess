...
public class ChunkVectorHelperTest extends UnitFessTestCase {
    ...
    protected boolean isUseOneTimeContainer() {
        // Several tests in this class (including the max_documents_per_run unlimited-default tests
        // below) write real system properties via ComponentUtil.getFessConfig().setSystemProperty();
        // systemProperties is a JVM-lifetime singleton, so reusing a cached container across test
        // methods would leak configuration between tests order-dependently.
        return true;
    }
    ...
    @Test
    public void test_getJobMaxDocumentsPerRun_defaultsToUnlimited() {
        final ChunkVectorHelper helper = new ChunkVectorHelper();
        assertEquals(-1, helper.getJobMaxDocumentsPerRun());
    }

    @Test
    public void test_isUnlimitedMaxDocuments() {
        final ChunkVectorHelper helper = new ChunkVectorHelper();

        ComponentUtil.getFessConfig().setSystemProperty("content_chunker.job.max_documents_per_run", "-1");
        assertTrue(helper.isUnlimitedMaxDocuments(helper.getJobMaxDocumentsPerRun()));

        ComponentUtil.getFessConfig().setSystemProperty("content_chunker.job.max_documents_per_run", "0");
        assertTrue(helper.isUnlimitedMaxDocuments(helper.getJobMaxDocumentsPerRun()));

        ComponentUtil.getFessConfig().setSystemProperty("content_chunker.job.max_documents_per_run", "5000");
        assertFalse(helper.isUnlimitedMaxDocuments(helper.getJobMaxDocumentsPerRun()));
    }
    ...
    public void test_jobLoopDefaults_matchNamingSummary() {
        ...
        assertEquals(-1, helper.getJobMaxDocumentsPerRun(), "the per-run document cap now defaults to unlimited (-1)");
    }
