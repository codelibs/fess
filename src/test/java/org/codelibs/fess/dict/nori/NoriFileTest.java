...
public class NoriFileTest extends UnitFessTestCase {
    private NoriFile noriFile;
    ...
    */
    ...
    public void setUp() throws Exception {
        super.setUp();
        noriFile = new NoriFile("1", "dummy", new Date());
        List<NoriItem> itemList = new ArrayList<>();
        itemList.add(new NoriItem(1, "token1", "seg1"));
        itemList.add(new NoriItem(2, "token2", "seg2"));
        itemList.add(new NoriItem(3, "token3", "seg3"));
        noriFile.noriItemList = itemList;
    }

    public void test_selectList() {
        final PagingList<NoriItem> itemList1 = noriFile.selectList(0, 20);
        ...
        final PagingList<NoriItem> itemList2 = noriFile.selectList(2, 2);
        ...
        assertEquals(0, noriFile.selectList(5, 5).size());
        assertEquals(0, noriFile.selectList(-1, 5).size());
    }

    public void test_selectList2() {
        final PagingList<NoriItem> itemList = noriFile.selectList(0, 5);
        for (int i = 0; i < itemList.size(); i++) {
            final NoriItem noriItem = itemList.get(i);
            assertEquals("token" + (i + 1), noriItem.getToken());
            assertEquals("seg" + (i + 1), noriItem.getSegmentation());
