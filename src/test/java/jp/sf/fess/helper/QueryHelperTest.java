...
public class QueryHelperTest extends S2TestCase {
    ...
    public void test_build() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("(title:QUERY1\\ QUERY2 OR content:QUERY1\\ QUERY2) "
                    + op + " (title:QUERY3 OR content:QUERY3)", queryHelper
                    .buildQuery("\"QUERY1 QUERY2\" QUERY3").getQuery());
        }
    }
    ...
    public void test_build_fullwidthSpace() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals(
                    "(title:QUERY1\\\u3000QUERY2 OR content:QUERY1\\\u3000QUERY2) "
                            + op + " (title:QUERY3 OR content:QUERY3)",
                    queryHelper
    ...
    public void test_build_prefix() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("mimetype:QUERY1 " + op
                    + " (title:QUERY2 OR content:QUERY2) " + op
                    + " (title:QUERY3 OR content:QUERY3)", queryHelper
                    .buildQuery("mimetype:QUERY1 QUERY2 QUERY3").getQuery());
            assertEquals("mimetype:QUERY1 " + op + " host:QUERY2 " + op
                    + " (title:QUERY3 OR content:QUERY3)", queryHelper
                    .buildQuery("mimetype:QUERY1 host:QUERY2 QUERY3")
                    .getQuery());
            assertEquals("mimetype:QUERY1\\ QUERY2 " + op
                    + " (title:QUERY3 OR content:QUERY3)", queryHelper
                    .buildQuery("mimetype:\"QUERY1 QUERY2\" QUERY3").getQuery());
        }
    }
    ...
    public void test_build_prefix_unknown() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("title:hoge\\:QUERY1 OR content:hoge\\:QUERY1",
                    queryHelper.buildQuery("hoge:QUERY1").getQuery());
            assertEquals("(title:hoge\\:QUERY1 OR content:hoge\\:QUERY1) " + op
                    + " (title:QUERY2 OR content:QUERY2)", queryHelper
                    .buildQuery("hoge:QUERY1 QUERY2").getQuery());
            assertEquals("(title:hoge\\:QUERY1 OR content:hoge\\:QUERY1) " + op
                    + " (title:QUERY2 OR content:QUERY2) " + op
                    + " (title:QUERY3 OR content:QUERY3)", queryHelper
                    .buildQuery("hoge:QUERY1 QUERY2 QUERY3").getQuery());
            assertEquals("(title:hoge\\:QUERY1 OR content:hoge\\:QUERY1) " + op
                    + " host:QUERY2 " + op
                    + " (title:QUERY3 OR content:QUERY3)", queryHelper
                    .buildQuery("hoge:QUERY1 host:QUERY2 QUERY3").getQuery());
            assertEquals(
                    "(title:hoge\\:QUERY1\\ QUERY2 OR content:hoge\\:QUERY1\\ QUERY2) "
                            + op + " (title:QUERY3 OR content:QUERY3)",
    ...
    public void test_build_roleType() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("role:guest", queryHelper.build("QUERY", true)
                    .getFilterQueries()[0]);

            assertEquals("(title:QUERY1 OR content:QUERY1) " + op
                    + " (title:QUERY2 OR content:QUERY2)",
                    queryHelper.build("QUERY1 QUERY2", true).getQuery());
            assertEquals("role:guest", queryHelper.build("QUERY1 QUERY2", true)
                    .getFilterQueries()[0]);
            ...
            assertEquals("role:guest OR role:admin",
                    queryHelper.build("QUERY", true).getFilterQueries()[0]);
    ...
    public void test_sortField() {
        ...
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("contentLength",
                    searchQuery.getSortFields()[0].getField());
            assertEquals(Constants.ASC,
                    searchQuery.getSortFields()[0].getOrder());
            ...
            assertEquals("lastModified",
                    searchQuery.getSortFields()[1].getField());
            assertEquals(Constants.ASC,
                    searchQuery.getSortFields()[1].getOrder());
            ...
            searchQuery = queryHelper.buildQuery(query);
            assertEquals("(title:QUERY OR content:QUERY) " + op
                    + " mimetype:QUERY1", searchQuery.getQuery());
            assertEquals(1, searchQuery.getSortFields().length);
            assertEquals("contentLength",
                    searchQuery.getSortFields()[0].getField());
            assertEquals(Constants.ASC,
    ...
    public void test_wildcardSearches() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("title:QUERY* OR content:QUERY*", queryHelper
                    .buildQuery("QUERY* ").getQuery());
            assertEquals("title:Q*ERY OR content:Q*ERY", queryHelper
                    .buildQuery(" Q*ERY").getQuery());

            assertEquals("(title:Q*ERY1 OR content:Q*ERY1) " + op
                    + " (title:Q*ERY2 OR content:Q*ERY2)", queryHelper
            ...
            assertEquals("(title:Q*ERY1\\ Q*ERY2 OR content:Q*ERY1\\ Q*ERY2) "
                    + op + " (title:Q*ERY3 OR content:Q*ERY3)", queryHelper
                    .buildQuery("\"Q*ERY1 Q*ERY2\" Q*ERY3").getQuery());
    ...
    public void test_proximitySearches() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("title:\"QUERY\"~10 OR content:\"QUERY\"~10",
                    queryHelper.buildQuery("QUERY~10").getQuery());
            assertEquals("title:\"QUERY\"~1 OR content:\"QUERY\"~1",
                    queryHelper.buildQuery("QUERY~1").getQuery());
            assertEquals("title:\"QUERY\"~5 OR content:\"QUERY\"~5",
                    queryHelper.buildQuery("QUERY~5.5").getQuery());
            assertEquals("(title:\"QUERY1\"~10 OR content:\"QUERY1\"~10) " + op
                    + " (title:QUERY2 OR content:QUERY2)", queryHelper
                    .buildQuery("QUERY1~10 QUERY2").getQuery());
            assertEquals("(title:\"QUERY1\"~5 OR content:\"QUERY1\"~5) " + op
                    + " (title:\"QUERY2\"~10 OR content:\"QUERY2\"~10)",
                    queryHelper.buildQuery("QUERY1~5 QUERY2~10").getQuery());

            assertEquals("mimetype:\"QUERY1\"~10",
                    queryHelper.buildQuery("mimetype:QUERY1~10").getQuery());
            assertEquals("mimetype:\"QUERY1\"~10 " + op
                    + " (title:QUERY2 OR content:QUERY2)", queryHelper
                    .buildQuery("mimetype:QUERY1~10 QUERY2").getQuery());
            ...
            assertEquals("title:\"QUERY1\"~10 OR content:\"QUERY1\"~10",
                    queryHelper.buildQuery("\"QUERY1~10\"").getQuery());

            assertEquals("title:\"QUERY1\"~10 OR content:\"QUERY1\"~10",
                    queryHelper.buildQuery("\"QUERY1~10a\"").getQuery());
        }
    }
    ...
    public void test_rangeSearches() {
        ...
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("lastModified:[20020101 TO 20030101]", queryHelper
                    .buildQuery("lastModified:[20020101 TO 20030101]")
                    .getQuery());
            assertEquals("(title:QUERY OR content:QUERY) " + op
                    + " lastModified:[20020101 TO 20030101]", queryHelper
                    .buildQuery("QUERY lastModified:[20020101 TO 20030101]")
                    .getQuery());
            assertEquals("title:{Aida TO Carmen} OR content:{Aida TO Carmen}",
                    queryHelper.buildQuery("{Aida TO Carmen}").getQuery());
            assertEquals("lastModified:{Aida TO Carmen}", queryHelper
                    .buildQuery("lastModified:{Aida TO Carmen}").getQuery());
            assertEquals("(title:QUERY OR content:QUERY) " + op
                    + " title:{Aida TO Carmen}",
                    queryHelper.buildQuery("QUERY title:{Aida TO Carmen}")
                            .getQuery());
            assertEquals("lastModified:[20020101 TO abc]", queryHelper
                    .buildQuery("lastModified:[20020101 TO abc]").getQuery());
            assertEquals("lastModified:[abc TO 20020101]", queryHelper
                    .buildQuery("lastModified:[abc TO 20020101]").getQuery());
            assertEquals("lastModified:[20020101 TO *]", queryHelper
                    .buildQuery("lastModified:[20020101 TO *]").getQuery());
            assertEquals("lastModified:[* TO 20020101]", queryHelper
                    .buildQuery("lastModified:[* TO 20020101]").getQuery());

            rangeQuery = "(content:[1 TO 2] OR content:[3 TO 4]) " + op
                    + " (content:[5 TO 6] OR content:[7 TO 8])";
            assertEquals(rangeQuery, queryHelper.buildQuery(rangeQuery)
                    .getQuery());
    ...
    public void test_boosting() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("(title:QUERY1^1000 OR content:QUERY1^1000) " + op
                    + " (title:QUERY2 OR content:QUERY2)", queryHelper
                    .buildQuery("QUERY1^1000 QUERY2").getQuery());
            assertEquals("(title:QUERY1^500 OR content:QUERY1^500) " + op
                    + " (title:QUERY2^1000 OR content:QUERY2^1000)",
            ...
            assertEquals("mimetype:QUERY1^1000 " + op
                    + " (title:QUERY2 OR content:QUERY2)", queryHelper
            ...
            assertEquals("title:QUERY1^1000 OR content:QUERY1^1000",
                    queryHelper.buildQuery("\"QUERY1^1000\"").getQuery());
        }
    }
    ...
    public void test_reserved() {
        for (int i = 0; i < Constants.RESERVED.length - 1; i++) {
            try {
                assertEquals("title:\\" + Constants.RESERVED[i]
                        + " OR content:\\" + Constants.RESERVED[i], queryHelper
                        .buildQuery(Constants.RESERVED[i]).getQuery());
            } catch (final InvalidQueryException e) {
        ...
        assertEquals("title:\\: OR content:\\:", queryHelper.buildQuery(":")
                .getQuery());
    }
    ...
    public void test_or() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("title:QUERY", queryHelper
                    .buildQuery("OR title:QUERY").getQuery());
            assertEquals("title:QUERY1 OR title:QUERY2", queryHelper
            ...
            assertEquals("mimetype:QUERY1 OR title:QUERY2", queryHelper
                    .buildQuery("mimetype:QUERY1 OR title:QUERY2").getQuery());
            assertEquals(
                    "(title:QUERY1 OR content:QUERY1) OR (title:QUERY2 OR content:QUERY2) "
                            + op + " (title:QUERY3 OR content:QUERY3)",
                    queryHelper.buildQuery("QUERY1 OR QUERY2 QUERY3")
                            .getQuery());
            assertEquals(
                    "(title:QUERY1 OR content:QUERY1) OR (title:QUERY2 OR content:QUERY2) OR (title:QUERY3 OR content:QUERY3)",
                    queryHelper.buildQuery("QUERY1 OR QUERY2 OR QUERY3")
                            .getQuery());
        }
    }
    ...
    public void test_and() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("title:QUERY",
                    queryHelper.buildQuery("AND title:QUERY").getQuery());
            assertEquals("title:QUERY1 AND title:QUERY2", queryHelper
                    .buildQuery("title:QUERY1 AND title:QUERY2").getQuery());
            assertEquals("(title:QUERY1 OR content:QUERY1) AND title:QUERY2",
                    queryHelper.buildQuery("QUERY1 AND title:QUERY2")
                            .getQuery());
            ...
            assertEquals(
                    "(title:QUERY1 OR content:QUERY1) AND (title:QUERY2 OR content:QUERY2) "
                            + op + " (title:QUERY3 OR content:QUERY3)",
                    queryHelper.buildQuery("QUERY1 AND QUERY2 QUERY3")
                            .getQuery());
            assertEquals(
                    "(title:QUERY1 OR content:QUERY1) "
                            + op
                            + " (title:QUERY2 OR content:QUERY2) AND (title:QUERY3 OR content:QUERY3)",
                    queryHelper.buildQuery("QUERY1 QUERY2 AND QUERY3")
                            .getQuery());
            assertEquals(
    ...
    public void test_not() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("NOT title:QUERY",
                    queryHelper.buildQuery("NOT title:QUERY").getQuery());
            assertEquals("(title:QUERY2 OR content:QUERY2) " + op
                    + " NOT (title:QUERY1 OR content:QUERY1)", queryHelper
                    .buildQuery("NOT QUERY1 QUERY2").getQuery());
            assertEquals("(title:QUERY2 OR content:QUERY2) " + op
                    + " NOT (title:QUERY1 OR content:QUERY1)", queryHelper
                    .buildQuery("NOT QUERY1 OR QUERY2").getQuery());
            assertEquals("NOT (title:QUERY1 OR content:QUERY1) " + op
                    + " NOT (title:QUERY2 OR content:QUERY2)", queryHelper
                    .buildQuery("NOT QUERY1 NOT QUERY2").getQuery());
            assertEquals("NOT (title:QUERY1 OR content:QUERY1) " + op
                    + " NOT (title:QUERY2 OR content:QUERY2)", queryHelper
                    .buildQuery("NOT QUERY1 OR NOT QUERY2").getQuery());
            assertEquals("(title:QUERY2 OR content:QUERY2) " + op
                    + " NOT (title:QUERY1 OR content:QUERY1) " + op
                    + " NOT (title:QUERY3 OR content:QUERY3)", queryHelper
                    .buildQuery("NOT QUERY1 QUERY2 NOT QUERY3").getQuery());
            assertEquals("NOT mimetype:QUERY",
                    queryHelper.buildQuery("NOT mimetype:QUERY").getQuery());
            assertEquals(
                    "NOT mimetype:QUERY1 " + op + " NOT title:QUERY2",
                    queryHelper.buildQuery(
                            "NOT mimetype:QUERY1 NOT title:QUERY2").getQuery());
            assertEquals("(title:QUERY2 OR content:QUERY2) " + op
                    + " NOT mimetype:QUERY1",
                    queryHelper.buildQuery("NOT mimetype:QUERY1 QUERY2")
                            .getQuery());
            assertEquals("mimetype:QUERY2 " + op
                    + " NOT (title:QUERY1 OR content:QUERY1)", queryHelper
    ...
    public void test_buildFacet_prefix_unknown() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("title:hoge\\:QUERY1 OR content:hoge\\:QUERY1",
                    queryHelper.buildFacetQuery("hoge:QUERY1"));
            assertEquals("(title:hoge\\:QUERY1 OR content:hoge\\:QUERY1) " + op
                    + " (title:QUERY2 OR content:QUERY2)",
    ...
    public void test_buildFacet_wildcardSearches() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("title:Q*ERY OR content:Q*ERY",
                    queryHelper.buildFacetQuery(" Q*ERY"));

            assertEquals("(title:Q*ERY1 OR content:Q*ERY1) " + op
                    + " (title:Q*ERY2 OR content:Q*ERY2)",
            ...
            assertEquals("(title:Q*ERY1\\ Q*ERY2 OR content:Q*ERY1\\ Q*ERY2) "
                    + op + " (title:Q*ERY3 OR content:Q*ERY3)",
    ...
    public void test_buildFacet_proximitySearches() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            // ~10
            assertEquals("title:\"QUERY\"~10 OR content:\"QUERY\"~10",
                    queryHelper.buildFacetQuery("QUERY~10"));
            assertEquals("title:\"QUERY\"~1 OR content:\"QUERY\"~1",
                    queryHelper.buildFacetQuery("QUERY~1"));
            assertEquals("title:\"QUERY\"~5 OR content:\"QUERY\"~5",
                    queryHelper.buildFacetQuery("QUERY~5.5"));
            assertEquals("(title:\"QUERY1\"~10 OR content:\"QUERY1\"~10) " + op
                    + " (title:QUERY2 OR content:QUERY2)",
            ...
            assertEquals("mimetype:\"QUERY1\"~10",
                    queryHelper.buildFacetQuery("mimetype:QUERY1~10"));
            assertEquals("mimetype:\"QUERY1\"~10 " + op
                    + " (title:QUERY2 OR content:QUERY2)",
            ...
            assertEquals("title:\"QUERY1\"~10 OR content:\"QUERY1\"~10",
                    queryHelper.buildFacetQuery("\"QUERY1~10\""));

            assertEquals("title:\"QUERY1\"~10 OR content:\"QUERY1\"~10",
                    queryHelper.buildFacetQuery("\"QUERY1~10a\""));
        }
    }
    ...
    public void test_buildFacet_rangeSearches() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals(
                    "lastModified:[20020101 TO 20030101]",
                    queryHelper
                            .buildFacetQuery("lastModified:[20020101 TO 20030101]"));
            assertEquals(
                    "(title:QUERY OR content:QUERY) " + op
                            + " lastModified:[20020101 TO 20030101]",
                    queryHelper
            ...
            // title:{Aida TO Carmen}
            assertEquals("title:{Aida TO Carmen} OR content:{Aida TO Carmen}",
                    queryHelper.buildFacetQuery("{Aida TO Carmen}"));
    ...
    public void test_buildFacet_boosting() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("title:QUERY^1000 OR content:QUERY^1000",
                    queryHelper.buildFacetQuery("QUERY^1000"));
            assertEquals("(title:QUERY1^1000 OR content:QUERY1^1000) " + op
                    + " (title:QUERY2 OR content:QUERY2)",
            ...
            assertEquals("mimetype:QUERY1^1000",
                    queryHelper.buildFacetQuery("mimetype:QUERY1^1000"));
            assertEquals("mimetype:QUERY1^1000 " + op
                    + " (title:QUERY2 OR content:QUERY2)",
            ...
            assertEquals("title:QUERY1^1000 OR content:QUERY1^1000",
                    queryHelper.buildFacetQuery("\"QUERY1^1000\""));
        }
    }
    ...
    public void test_buildFacet_or() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("title:QUERY",
                    queryHelper.buildFacetQuery("OR title:QUERY"));
            assertEquals("title:QUERY1 OR title:QUERY2",
                    queryHelper.buildFacetQuery("title:QUERY1 OR title:QUERY2"));
            assertEquals("(title:QUERY1 OR content:QUERY1) OR title:QUERY2",
                    queryHelper.buildFacetQuery("QUERY1 OR title:QUERY2"));
    ...
    public void test_buildFacet_and() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals(
                    "mimetype:QUERY1 AND title:QUERY2",
                    queryHelper
    ...
    public void test_buildFacet_not() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("NOT title:QUERY",
                    queryHelper.buildFacetQuery("NOT title:QUERY"));
            assertEquals("(title:QUERY2 OR content:QUERY2) " + op
                    + " NOT (title:QUERY1 OR content:QUERY1)",
                    queryHelper.buildFacetQuery("NOT QUERY1 QUERY2"));
            assertEquals("(title:QUERY2 OR content:QUERY2) " + op
                    + " NOT (title:QUERY1 OR content:QUERY1)",
                    queryHelper.buildFacetQuery("NOT QUERY1 OR QUERY2"));
            assertEquals("NOT (title:QUERY1 OR content:QUERY1) " + op
                    + " NOT (title:QUERY2 OR content:QUERY2)",
            ...
            assertEquals("(title:QUERY2 OR content:QUERY2) " + op
                    + " NOT (title:QUERY1 OR content:QUERY1) " + op
                    + " NOT (title:QUERY3 OR content:QUERY3)",
                    queryHelper.buildFacetQuery("NOT QUERY1 QUERY2 NOT QUERY3"));
            assertEquals("NOT mimetype:QUERY",
                    queryHelper.buildFacetQuery("NOT mimetype:QUERY"));
            assertEquals(
                    "NOT mimetype:QUERY1 " + op + " NOT title:QUERY2",
                    queryHelper
    ...
    public void test_inner_query() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("((title:bbb OR content:bbb) " + op
                    + " (title:ccc OR content:ccc)) " + op
                    + " (title:ddd OR content:ddd)",
                    queryHelper.buildQuery("(bbb ccc) ddd").getQuery());
            assertEquals(
                    "((title:bbb OR content:bbb) AND (title:ccc OR content:ccc)) "
                            + op + " (title:ddd OR content:ddd)", queryHelper
                            .buildQuery("(bbb AND ccc) ddd").getQuery());
            assertEquals(
            ...
            assertEquals(
                    "((title:bbb OR content:bbb) OR (title:ccc OR content:ccc)) "
                            + op + " (title:ddd OR content:ddd)", queryHelper
                            .buildQuery("(bbb OR ccc) ddd").getQuery());
            assertEquals(
            ...
            assertEquals(
                    "(title:aaa OR content:aaa) "
                            ...
                            + " ((title:bbb OR content:bbb) AND (title:ccc OR content:ccc)) "
                            + op + " (title:ddd OR content:ddd)", queryHelper
                            .buildQuery("aaa (bbb AND ccc) ddd").getQuery());
            assertEquals(
            ...
            assertEquals(
                    "(title:aaa OR content:aaa) "
                            ...
                            + " ((title:bbb OR content:bbb) OR (title:ccc OR content:ccc)) "
                            + op + " (title:ddd OR content:ddd)", queryHelper
                            .buildQuery("aaa (bbb OR ccc) ddd").getQuery());
            assertEquals(
            ...
            assertEquals(
                    ...
                    queryHelper.buildQuery("aaa OR (bbb OR ccc) OR ddd")
                            .getQuery());

            assertEquals("(title:aaa OR content:aaa) " + op + " (label:bbb "
                    + op + " (title:ccc OR content:ccc))", queryHelper
                    .buildQuery("aaa (label:bbb ccc)").getQuery());
            assertEquals("(title:aaa OR content:aaa) " + op
                    + " (label:bbb AND (title:ccc OR content:ccc))",
                    queryHelper.buildQuery("aaa (label:bbb AND ccc)")
                            .getQuery());
            assertEquals("(title:aaa OR content:aaa) " + op
                    + " (label:bbb OR (title:ccc OR content:ccc))", queryHelper
                    .buildQuery("aaa (label:bbb OR ccc)").getQuery());

            assertEquals("(title:aaa OR content:aaa) " + op
                    + " ((title:bbb OR content:bbb) " + op + " label:ccc)",
                    queryHelper.buildQuery("aaa (bbb label:ccc)").getQuery());
            assertEquals("(title:aaa OR content:aaa) " + op
                    + " ((title:bbb OR content:bbb) AND label:ccc)",
                    queryHelper.buildQuery("aaa (bbb AND label:ccc)")
                            .getQuery());
            assertEquals("(title:aaa OR content:aaa) " + op
                    + " ((title:bbb OR content:bbb) OR label:ccc)", queryHelper
                    .buildQuery("aaa (bbb OR label:ccc)").getQuery());

            assertEquals("(title:aaa OR content:aaa) " + op
                    + " ((title:bbb OR content:bbb) " + op
                    + " NOT (title:ccc OR content:ccc))", queryHelper
                    .buildQuery("aaa (bbb NOT ccc)").getQuery());
        }
    }
    ...
    public void test_inner_facetQuery() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("(title:aaa OR content:aaa) " + op
                    + " ((title:bbb OR content:bbb) OR label:ccc)",
                    queryHelper.buildFacetQuery("aaa (bbb OR label:ccc)"));

            assertEquals("(title:aaa OR content:aaa) " + op
                    + " ((title:bbb OR content:bbb) " + op
                    + " NOT (title:ccc OR content:ccc))",
    ...
    public void test_quote_query() {
        final String[] formats = { "%s", " %s ", "aaa %s", "aaa OR %s",
                "NOT %s", "%s bbb", "%s bbb", "%s OR bbb", "aaa (%s)",
                "(%s) bbb", "aaa (%s) bbb" };
    ...
    private void assertException(final String format, final String query,
        ...
        try {
            final String ret = queryHelper.buildQuery(
    ...
    public void test_inurl() {
        for (final String op : new String[] { "AND", "OR" }) {
            ...
            assertEquals("url:*QUERY*", queryHelper.buildQuery("inurl:QUERY")
                    .getQuery());
            assertEquals("url:*QUERY1* " + op + " url:*QUERY2*", queryHelper
                    .buildQuery("inurl:QUERY1 inurl:QUERY2").getQuery());
            assertEquals("(title:aaa OR content:aaa) " + op + " url:*QUERY1* "
                    + op + " url:*QUERY2*",
                    queryHelper.buildQuery("aaa inurl:QUERY1 inurl:QUERY2")
                            .getQuery());
            assertEquals("url:*QUERY*",
                    queryHelper.buildQuery("inurl:\"QUERY\"").getQuery());
        }
    }
    ...
    private void assertExceptionOnFacetQuery(final String format,
        ...
        try {
            final String ret = queryHelper.buildFacetQuery(String.format(
                    format, query));
            fail("format: " + format + ", query: " + query + ", ret: " + ret);
        } catch (final InvalidQueryException e) {
            if (!messageCode.equals(e.getMessageCode())) {
