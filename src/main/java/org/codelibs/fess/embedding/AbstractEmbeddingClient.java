...
public abstract class AbstractEmbeddingClient implements EmbeddingClient {
     ...
     * {@code system.properties} would otherwise make Fess unbootable. The other caller is
     ...
     * Read through {@code FessProp.getSystemProperty}, the mutable {@code systemProperties}
     * component (set via {@code conf/system.properties} or a {@code -Dfess.system.<key>} JVM
     * argument) -- the same channel as every other {@code content_chunker.*} key, not the
     * LastaFlute {@code fess_config.properties} channel loaded once at container boot. A change
     * is picked up without a restart.
     *
     * <p>A key that is present but set to an empty string yields {@code ""}, not
     * {@code defaultValue}: only an absent key falls back to the default. This is intended
     * behavior, not a bug -- subclasses rely on an explicit empty value (e.g. a prefix key set to
     * {@code ""}) to disable a feature, distinct from the key being unset entirely.</p>
     *
