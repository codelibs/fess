<%@page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8"%><!DOCTYPE html>
${fe:html(true)}
<head>
    <meta charset="UTF-8">
    <title><la:message key="labels.admin_brand_title"/> | <la:message key="labels.search_list_configuration"/></title>
    <jsp:include page="/WEB-INF/view/common/admin/head.jsp"></jsp:include>
</head>
<body class="fads-admin-layout">
<div class="fads-layout-wrapper">
    <jsp:include page="/WEB-INF/view/common/admin/header.jsp"></jsp:include>
    <jsp:include page="/WEB-INF/view/common/admin/sidebar.jsp">
        <jsp:param name="menuCategoryType" value="log"/>
        <jsp:param name="menuType" value="searchList"/>
    </jsp:include>
    <main class="fads-main-content">
        <div class="fads-page-header">
            <div >
                <div class="fads-d-flex fads-align-center" style="flex-wrap:wrap;gap:var(--ds-space-100)">
                    <div class="fads-col-sm-6">
                        <h1>
                            <la:message key="labels.search_list_configuration"/>
                        </h1>
                    </div>
                    <div class="fads-col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item active"><la:link href="/admin/searchlist/search?q=${f:u(q)}">
                                <la:message key="labels.search_list_configuration"/>
                            </la:link></li>
                        </ol>
                    </div>
                </div>
            </div>
        </div>
        <section class="content">
            <la:form action="/admin/searchlist/">
                <la:hidden property="crudMode"/>
                <la:hidden property="q"/>
                <c:if test="${crudMode==2}">
                    <la:hidden property="id"/>
                    <la:hidden property="seqNo"/>
                    <la:hidden property="primaryTerm"/>
                </c:if>
                <div class="fads-row">
                    <div class="fads-col-md-12">
                        <div class="fads-card <c:if test="${crudMode == 1 || crudMode == 2}">card-success</c:if>">
                            <div class="fads-card-header">
                                <h3 class="fads-card-title">
                                    <c:if test="${crudMode == null}">
                                        <la:message key="labels.crud_title_list"/>
                                    </c:if>
                                    <c:if test="${crudMode == 1}">
                                        <la:message key="labels.crud_title_create"/>
                                    </c:if>
                                    <c:if test="${crudMode == 2}">
                                        <la:message key="labels.crud_title_edit"/>
                                    </c:if>
                                    <c:if test="${crudMode == 3}">
                                        <la:message key="labels.crud_title_delete"/>
                                    </c:if>
                                    <c:if test="${crudMode == 4}">
                                        <la:message key="labels.crud_title_details"/>
                                    </c:if>
                                </h3>
                                <div class="fads-card-tools">
                                    <div class="btn-group">
                                        <c:choose>
                                            <c:when test="${crudMode == null}">
                                                <la:link href="createnew" styleClass="fads-btn fads-btn-success fads-btn-compact">
                                                    <i class="fa fa-plus" aria-hidden="true"></i>
                                                    <la:message key="labels.crud_link_create"/>
                                                </la:link>
                                            </c:when>
                                            <c:otherwise>
                                                <la:link href="/admin/searchlist/search?q=${f:u(q)}"
                                                         styleClass="fads-btn fads-btn-primary fads-btn-compact">
                                                    <i class="fa fa-th-list" aria-hidden="true"></i>
                                                    <la:message key="labels.crud_link_list"/>
                                                </la:link>
                                            </c:otherwise>
                                        </c:choose>
                                    </div>
                                </div>
                            </div>
                            <div class="fads-card-body">
                                <div>
                                    <la:info id="msg" message="true">
                                        <div class="fads-banner fads-banner-info">${msg}</div>
                                    </la:info>
                                    <la:errors property="_global"/>
                                </div>
                                <c:if test="${crudMode==2}">
                                    <div class="fads-form-field">
                                        <label class="fads-label">_id</label>
                                        <div class="fads-col-sm-9">${f:h(id)}</div>
                                    </div>
                                    <div class="fads-form-field">
                                        <label for="doc.doc_id" class="fads-label">doc_id</label>
                                        <div class="fads-col-sm-9">
                                                ${f:h(doc.doc_id)}
                                            <la:hidden styleId="doc.doc_id" property="doc.doc_id"/>
                                        </div>
                                    </div>
                                </c:if>
                                <div class="fads-form-field">
                                    <label for="doc.url" class="fads-label">url</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.url"/>
                                        <la:text styleId="doc.url"
                                                 property="doc.url" styleClass="fads-textfield"
                                                 required="required" data-validation="required"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.title" class="fads-label">title</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.title"/>
                                        <la:text styleId="doc.title"
                                                 property="doc.title" styleClass="fads-textfield"
                                                 required="required" data-validation="required"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.role" class="fads-label">role</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.role"/>
                                        <la:textarea styleId="doc.role"
                                                     property="doc.role" styleClass="fads-textfield"
                                                     data-validation-help="1(username) | 2(groupname) | R(rolename)  e.g. Rguest"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.boost" class="fads-label">boost</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.boost"/>
                                        <la:text styleId="doc.boost"
                                                 property="doc.boost" styleClass="fads-textfield"
                                                 title="Floating point number" required="required"
                                                 data-validation="custom"
                                                 data-validation-regexp="(\+|\-)?\d+(\.\d+)?((e|E)(\+|\-)?\d+)?"
                                                 data-validation-help="number (Float)"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.label" class="fads-label">label</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.label"/>
                                        <la:textarea styleId="doc.label" property="doc.label" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.lang" class="fads-label">lang</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.lang"/>
                                        <la:textarea styleId="doc.lang" property="doc.lang" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.mimetype" class="fads-label">mimetype</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.mimetype"/>
                                        <la:text styleId="doc.mimetype" property="doc.mimetype" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.filetype" class="fads-label">filetype</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.filetype"/>
                                        <la:text styleId="doc.filetype" property="doc.filetype" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.filename" class="fads-label">filename</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.filename"/>
                                        <la:text styleId="doc.filename" property="doc.filename" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.content" class="fads-label">content</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.content"/>
                                        <la:text styleId="doc.content" property="doc.content" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.has_cache" class="fads-label">has_cache</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.has_cache"/>
                                        <la:text styleId="doc.has_cache" property="doc.has_cache" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.cache" class="fads-label">cache</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.cache"/>
                                        <la:text styleId="doc.cache" property="doc.cache" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.digest" class="fads-label">digest</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.digest"/>
                                        <la:text styleId="doc.digest" property="doc.digest" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.host" class="fads-label">host</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.host"/>
                                        <la:text styleId="doc.host" property="doc.host" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.site" class="fads-label">site</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.site"/>
                                        <la:text styleId="doc.site" property="doc.site" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.segment" class="fads-label">segment</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.segment"/>
                                        <la:text styleId="doc.segment" property="doc.segment" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.config_id" class="fads-label">config_id</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.config_id"/>
                                        <la:text styleId="doc.config_id" property="doc.config_id" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.parent_id" class="fads-label">parent_id</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.parent_id"/>
                                        <la:text styleId="doc.parent_id" property="doc.parent_id" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.content_length" class="fads-label">content_length</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.content_length"/>
                                        <la:text styleId="doc.content_length"
                                                 property="doc.content_length"
                                                 styleClass="fads-textfield" title="Integer"
                                                 data-validation="custom" data-validation-regexp="^(\d+)?$"
                                                 data-validation-help="number (Integer)"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.favorite_count" class="fads-label">favorite_count</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.favorite_count"/>
                                        <la:text styleId="doc.favorite_count"
                                                 property="doc.favorite_count"
                                                 styleClass="fads-textfield" title="Integer"
                                                 data-validation="custom" data-validation-regexp="^(\d+)?$"
                                                 data-validation-help="number (Integer)"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.click_count" class="fads-label">click_count</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.click_count"/>
                                        <la:text styleId="doc.click_count"
                                                 property="doc.click_count" styleClass="fads-textfield"
                                                 title="Integer" data-validation="custom"
                                                 data-validation-regexp="^(\d+)?$"
                                                 data-validation-help="number (Integer)"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.created" class="fads-label">created</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.created"/>
                                        <la:text styleId="doc.created"
                                                 property="doc.created" styleClass="fads-textfield"
                                                 title="yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
                                                 data-validation="custom"
                                                 data-validation-regexp="(^$|^[1-9]\d{3}\-\d\d\-\d\dT\d\d\:\d\d\:\d\d\.\d{3}Z$)"
                                                 data-validation-help="yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.timestamp" class="fads-label">timestamp</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.timestamp"/>
                                        <la:text styleId="doc.timestamp"
                                                 property="doc.timestamp" styleClass="fads-textfield"
                                                 title="yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
                                                 data-validation="custom"
                                                 data-validation-regexp="(^$|^[1-9]\d{3}\-\d\d\-\d\dT\d\d\:\d\d\:\d\d\.\d{3}Z$)"
                                                 data-validation-help="yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.last_modified" class="fads-label">last_modified</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.last_modified"/>
                                        <la:text styleId="doc.last_modified"
                                                 property="doc.last_modified" styleClass="fads-textfield"
                                                 title="yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
                                                 data-validation="custom"
                                                 data-validation-regexp="(^$|^[1-9]\d{3}\-\d\d\-\d\dT\d\d\:\d\d\:\d\d\.\d{3}Z$)"
                                                 data-validation-help="yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.expires" class="fads-label">expires</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.expires"/>
                                        <la:text styleId="doc.expires"
                                                 property="doc.expires" styleClass="fads-textfield"
                                                 title="yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
                                                 data-validation="custom"
                                                 data-validation-regexp="(^$|^[1-9]\d{3}\-\d\d\-\d\dT\d\d\:\d\d\:\d\d\.\d{3}Z$)"
                                                 data-validation-help="yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"/>
                                    </div>
                                </div>
                                <div class="fads-form-field">
                                    <label for="doc.virtual_host" class="fads-label">virtual_host</label>
                                    <div class="fads-col-sm-9">
                                        <la:errors property="doc.virtual_host"/>
                                        <la:textarea styleId="doc.virtual_host" property="doc.virtual_host" styleClass="fads-textfield"/>
                                    </div>
                                </div>
                            </div>
                            <div class="fads-card-footer">
                                <c:if test="${crudMode == 1}">
                                    <la:link href="/admin/searchlist/search?q=${f:u(q)}" styleClass="fads-btn fads-btn-default">
                                        <i class="fa fa-arrow-circle-left" aria-hidden="true"></i>
                                        <la:message key="labels.crud_button_back"/>
                                    </la:link>
                                    <button type="submit" class="fads-btn fads-btn-success" name="create"
                                            value="<la:message key="labels.crud_button_create" />">
                                        <i class="fa fa-plus" aria-hidden="true"></i>
                                        <la:message key="labels.crud_button_create"/>
                                    </button>
                                </c:if>
                                <c:if test="${crudMode == 2}">
                                    <la:link href="/admin/searchlist/search?q=${f:u(q)}" styleClass="fads-btn fads-btn-default">
                                        <i class="fa fa-arrow-circle-left" aria-hidden="true"></i>
                                        <la:message key="labels.crud_button_back"/>
                                    </la:link>
                                    <button type="submit" class="fads-btn fads-btn-success" name="update"
                                            value="<la:message key="labels.crud_button_update" />">
                                        <i class="fa fa-pencil-alt" aria-hidden="true"></i>
                                        <la:message key="labels.crud_button_update"/>
                                    </button>
                                </c:if>
                            </div>
                        </div>
                    </div>
                </div>
            </la:form>
        </section>
    </main>
    <jsp:include page="/WEB-INF/view/common/admin/footer.jsp"></jsp:include>
</div>
<jsp:include page="/WEB-INF/view/common/admin/foot.jsp"></jsp:include>
<script src="${fe:url('/js/admin/plugins/form-validator/jquery.form-validator.min.js')}"
        type="text/javascript"></script>
<script src="${fe:url('/js/admin/searchlist.js')}" type="text/javascript"></script>
</body>
${fe:html(false)}
