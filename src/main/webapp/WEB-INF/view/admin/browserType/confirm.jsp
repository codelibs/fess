<%@page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8"%><tiles:insert template="/WEB-INF/view/common/admin/layout.jsp"
	...
	<tiles:put name="body" type="string">
		...
		<h3>
			<bean:message key="labels.browsertype_title_confirm" />
		</h3>
			...
			<div>
				<ul class="pills">
					<li><s:link href="index">
							<bean:message key="labels.browsertype_link_list" />
						</s:link></li>
					<c:if test="${crudMode == 1}">
					<li class="active"><a href="#"><bean:message
								key="labels.browsertype_link_create" /></a></li>
					</c:if>
					<c:if test="${crudMode == 2}">
					<li class="active"><a href="#"><bean:message
								key="labels.browsertype_link_update" /></a></li>
					</c:if>
					<c:if test="${crudMode == 3}">
					<li class="active"><a href="#"><bean:message
								key="labels.browsertype_link_delete" /></a></li>
					</c:if>
					<c:if test="${crudMode == 4}">
					<li class="active"><a href="#"><bean:message
								key="labels.browsertype_link_confirm" /></a></li>
					</c:if>
				</ul>
			</div>
			...
			<div>
				...
				<table class="bordered-table zebra-striped" style="width: 500px;">
					<tbody>
						...
						<tr>
							<th><bean:message key="labels.browsertype_value" /></th>
						...
						<tr>
							<td colspan="2"><c:if test="${crudMode == 1}">
									<input type="submit" class="btn mini" name="create"
										value="<bean:message key="labels.browsertype_button_create"/>" />
									<input type="submit" class="btn mini" name="editagain"
										value="<bean:message key="labels.browsertype_button_back"/>" />
								</c:if> <c:if test="${crudMode == 2}">
									<input type="submit" class="btn mini" name="update"
										value="<bean:message key="labels.browsertype_button_update"/>" />
									<input type="submit" class="btn mini" name="editagain"
										value="<bean:message key="labels.browsertype_button_back"/>" />
								</c:if> <c:if test="${crudMode == 3}">
									<input type="submit" class="btn mini" name="delete"
										value="<bean:message key="labels.browsertype_button_delete"/>" />
									...
									<input type="submit" class="btn mini" name="editfromconfirm"
										value="<bean:message key="labels.browsertype_button_edit"/>" />
