...
통합 테스트에는 OpenSearch와 함께 실행 중인 Fess 서버가 필요합니다. 다음 단계를 따르세요:

#### 1. Fess 빌드

    $ mvn antrun:run  # OpenSearch 플러그인 다운로드 (아직 하지 않은 경우)
    $ mvn package     # 패키지 빌드
...
Fess가 준비될 때까지 기다립니다 (최대 60초 정도 걸릴 수 있습니다):

    $ curl -s "http://localhost:8080/api/v1/health"

Fess가 준비되면 JSON 응답이 표시됩니다.

#### 3. 테스트 데이터 클론

SearchApiTests에 필요합니다:
